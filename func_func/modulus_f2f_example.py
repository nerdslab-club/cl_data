...
def create_f2f_modulus_example(count: int, identifier: int | None):
    examples = []
...
def __random_io_operation(num1: int, num2: int, identifier: int | None, prev_choice=None) -> str:
    explanations = [
        f"##modulus({num1}, {num2})",
        f"##subtraction({num1}, ##multiplication({num2}, ##floor_division({num1}, {num2})))",
    ]
...
if __name__ == "__main__":
    print(
        Utility.create_sample_from_example(
            create_f2f_modulus_example(2), TaskTypes.FUNC_TO_FUNC_TRANSLATION
        )
    )
