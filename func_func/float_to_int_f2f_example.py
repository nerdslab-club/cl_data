...
def create_f2f_float_to_int_example(count: int, identifier: int | None):
    examples = []
...
def __random_io_operation(num1: float, identifier: int | None, prev_choice=None) -> str:
    explanations = [
        f"##float_to_int({num1})",
        f"##float_to_int(##int_to_float(##float_to_int({num1})))",
    ]
...
if __name__ == "__main__":
    print(
        Utility.create_sample_from_example(
            create_f2f_float_to_int_example(2), TaskTypes.FUNC_TO_FUNC_TRANSLATION
        )
    )
