from src.random_value_generator import RandomValueGenerator

import random

from src.constants import TaskTypes
from src.utility import Utility


def create_f2f_average_example(count: int):
    examples = []
    for _ in range(count):
        list1 = RandomValueGenerator.generate_random_list()
        choice_one = __random_io(list1)
        choice_two = __random_io(list1)
        examples.append({
            "inputStr": Utility.remove_spaces(choice_one),
            "outputStr": Utility.remove_spaces(choice_two)
        })
    return examples


def __random_io(list1: list) -> str:
    explanations = [
        f"##average({list1})",
        f"##mean({list1})",
        f"##division(##sum({list1}),##length({list1}))"
    ]
    return random.choice(explanations)


if __name__ == "__main__":
    print(Utility.create_sample_from_example(create_f2f_average_example(2), TaskTypes.FUNC_TO_FUNC_TRANSLATION))

