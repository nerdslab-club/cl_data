...
def create_f2f_multiplication_example(count: int, identifier: int | None):
    examples = []
    for i in range(count):
        num1 = RandomValueGenerator.generate_random_integer()
        num2 = RandomValueGenerator.generate_random_integer()
        choice_one = __random_io_multiplication(num1, num2, (None if identifier is None else identifier+i))
        choice_two = __random_io_multiplication(num1, num2, (None if identifier is None else identifier+i), choice_one)
        examples.append(
            {
...
def __random_io_multiplication(num1: int, num2: int, identifier: int | None, prev_choice=None) -> str:
    explanations = [
