...
def create_f2f_addition_example(count: int):
    examples = []
    for _ in range(count):
        num1 = RandomValueGenerator.generate_random_integer()
        num2 = RandomValueGenerator.generate_random_integer()
        choice_one = __random_io_addition(num1, num2)
        choice_two = __random_io_addition(num1, num2, choice_one)
        examples.append(
            {
...
def __random_io_addition(num1: int, num2: int, prev_choice=None) -> str:
    explanations = [
