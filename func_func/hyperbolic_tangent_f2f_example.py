...
def create_f2f_hyperbolic_example(count: int, identifier: int | None):
    examples = []
    for i in range(count):
        num1 = RandomValueGenerator.generate_random_float(0, 19)
...
def __random_io_operation(num1: float, identifier: int | None, prev_choice=None) -> str:
    explanations = [
        f"##hyperbolic_tangent({num1})",
        f"##division(##hyperbolic_sine({num1}), ##hyperbolic_cosine({num1}))",
    ]
...
if __name__ == "__main__":
    print(
        Utility.create_sample_from_example(
            create_f2f_hyperbolic_example(2), TaskTypes.FUNC_TO_FUNC_TRANSLATION
        )
    )
