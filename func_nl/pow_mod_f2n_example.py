...
def create_f2n_pow_mod_example(count: int, identifier: int | None):
    ...
    for i in range(count):
        x = RandomValueGenerator.generate_random_integer(1, 99, seed=(None if identifier is None else identifier+i))
        y = RandomValueGenerator.generate_random_integer(1, 20, seed=(None if identifier is None else identifier+i+1))
        mod = RandomValueGenerator.generate_random_integer(2, 99, seed=(None if identifier is None else identifier+i+2))
        examples.append(
            {
