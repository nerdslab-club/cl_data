...
def create_masked_token_batch_eleven_example(count: int, nlp: spacy.Language):
    examples = []
    for _ in range(count):
        paragraphs = __get_batch_eleven_example_paragraph()
...
def create_next_token_batch_eleven_example(count: int, nlp: spacy.Language):
    examples = []
    for _ in range(count):
        paragraphs = __get_batch_eleven_example_paragraph()
        for paragraph in paragraphs:
...
def __get_batch_eleven_example_paragraph():
...
if __name__ == "__main__":
    masked_example = create_masked_token_batch_eleven_example(1, Utility.get_spacy_nlp())
    ...
    next_token_example = create_next_token_batch_eleven_example(1, Utility.get_spacy_nlp())
