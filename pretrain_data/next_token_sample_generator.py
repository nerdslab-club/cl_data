...
class NextTokenSamples:
    ...
    def get_next_random_next_token_sample(self):
        random_generator_index = random.randint(0, len(self.next_token_example_generators)-1)
        selected_generator = list(self.next_token_example_generators.values())[random_generator_index]
        return Utility.create_sample_from_example(selected_generator(1), NextTokenSamples.TASK_TYPE)
    ...
    def __set_next_token_samples(self, each_example_count: int):
        for key, generator in self.next_token_example_generators.items():
            self.next_token_samples.extend(
                Utility.create_sample_from_example(generator(each_example_count), NextTokenSamples.TASK_TYPE),
            )
